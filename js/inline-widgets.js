...
export function initInlineWidgets(cm) {
  ...
  function placeWidgets() {
    ...
    function visitCall(call) {
      if (
        ...
        !turtleNames.has(call.callee.object.name)
      ) return;
      ...
      for (const arg of call.arguments) {
        ...
        if (typeof arg.value === "string" && method === "color") {
  ...
  function updateGhost() {
    ...
    const turtleNames = getTurtleNames(cm.getValue());
    if (!turtleNames.has(m[1])) return;
