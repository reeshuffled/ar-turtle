...
export function textToBlocks(code, workspace) {
  ...
  // Walk top-level statements, building a chain of blocks
  const topBlocks = [];
  for (const node of ast.body) {
    const blocks = stmtToBlocks(node, varName, workspace);
    topBlocks.push(...blocks);
  }

  // Chain all top-level blocks together and position the first one
  if (topBlocks.length > 0) {
    topBlocks[0].moveBy(x, y);
...
function stmtToBlocks(node, varName, workspace) {
  ...
  return methodToBlocks(method, args, varName, workspace);
}

function methodToBlocks(method, args, varName, workspace) {
  const block = makeMethodBlock(method, args, varName, workspace);
  if (!block) return [];
  return [block];
}

function makeMethodBlock(method, args, varName, workspace) {
  switch (method) {
    case 'forward':
      return makeNumBlock('turtle_forward', 'AMOUNT', numVal(args[0]), workspace);
    case 'backward':
      return makeNumBlock('turtle_backward', 'AMOUNT', numVal(args[0]), workspace);
    case 'right':
      return makeAngleBlock('turtle_right', numVal(args[0]), workspace);
    case 'left':
      return makeAngleBlock('turtle_left', numVal(args[0]), workspace);
    case 'disc':
      return makeNumBlock('turtle_disc', 'RADIUS', numVal(args[0]), workspace);
    case 'circle':
      return makeNumBlock('turtle_circle', 'RADIUS', numVal(args[0]), workspace);
    case 'thickness':
      return makeSliderBlock('turtle_thickness', numVal(args[0]), workspace);
    case 'pu':
      return makeSimpleBlock('turtle_pen_up', workspace);
    case 'pd':
      return makeSimpleBlock('turtle_pen_down', workspace);
    case 'home':
      return makeSimpleBlock('turtle_home', workspace);
    case 'clear':
      return makeSimpleBlock('turtle_clear', workspace);
    case 'clean':
      return makeSimpleBlock('turtle_clean', workspace);
    case 'color':
      return makeColorBlock(args[0], workspace);
    case 'repeat':
      return makeRepeatBlock(args, varName, workspace);
    default:
      return null;
  }
}
...
function makeSimpleBlock(type, workspace) {
...
function makeNumBlock(type, inputName, value, workspace) {
  const block = workspace.newBlock(type);
  if (value !== null) {
...
function makeAngleBlock(type, value, workspace) {
  const block = workspace.newBlock(type);
...
function makeSliderBlock(type, value, workspace) {
  const block = workspace.newBlock(type);
...
function makeColorBlock(argNode, workspace) {
...
function makeRepeatBlock(args, varName, workspace) {
  ...
  if (bodyFn) {
    ...
    for (const stmt of bodyStmts) {
      const bs = stmtToBlocks(stmt, varName, workspace);
      innerBlocks.push(...bs);
    }
